...
export default class RaffleView extends Vue {
    ...
    public created() {
        ...
        this.hubConnection.on("UserConnected", (result: UserConnection) => {
            const nameExists = this.joinedUsers.map(x => x.Name.toLowerCase()).indexOf(this.squareName.toLowerCase()) > -1;
            if (nameExists) {
                return;
            }
    ...
    public async mounted() {
        ...
        if (cookie) {
            ...
            if (userNumbers) {
                vueContext.selectedSquares = userNumbers.data.map((x: any) => parseInt(x));
            }
        } else {
    ...
    public squareClicked(square: Square) {
        ...
        if (doIHaveThisNumber) {
            ...
            const indexOfTakenSquare = this.takenSquares.indexOf(square.$props.squareNumber);
