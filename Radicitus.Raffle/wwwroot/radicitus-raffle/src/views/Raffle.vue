  ...
  <div class="section">
    ...
    <b-modal :can-cancel="false" :active.sync="isNameModalActive">
      <div class="box">
        ...
        <div class="has-text-centered">
          <p class="is-size-5">Tell us who you are so you can pick your numbers.</p>
        </div>
        <div class="has-text-centered">
          <div class="field">
            <div class="control">
    ...
    <div v-bind:class="{'whos-here-shown': isSlideoutShown}" class="whos-here-container">
      <div class="box whos-here">
        <div v-if="joinedUsers.length < 1">Looks like there isn't anyone else in here at the moment.</div>
        <div v-for="user in joinedUsers" v-bind:key="user.ConnectionId">{{user.Name}}</div>
      </div>
    </div>
  </div>
</template>
