...
export default class RadRaffleService {
    ...
    public async createRaffle(raffle: RadRaffle): Promise<AxiosResponse<RadRaffle>> {
        return await axios.post<RadRaffle>(`${this.url}/api/raffle/create`, raffle, {
            headers: this.headers
        });
    }

    public async getRaffleNumbersForUser(id: number, username: string): Promise<AxiosResponse<number[]>> {
        return await axios.get(`${this.url}/api/raffle/${id}/${username}/numbers`, {
            headers: this.headers
        });
    }

    public async getNumbersForRaffle(id: number): Promise<AxiosResponse<RaffleNumberSelection[]>> {
    ...
    public async chooseWinner(id: number): Promise<AxiosResponse<RaffleNumberSelection>> {
